...
class TestMocking:
    ...
    def test_mocked_probability_all_patterns(self, monkeypatch, mock_probs, majority_key, desc):
        ...
        # 許容誤差を考慮
        assert counts[majority_key] > (expected_majority_count * 0.9), \
            f"多数派のアイテム'{majority_key}'の出現回数({counts[majority_key]})が期待値({expected_majority_count})の90%を下回りました"
        print(f"[テスト成功] {majority_key}が期待通りに多数出現しました (出現回数: {counts[majority_key]}) ")
    ...
    def test_mock_random_choices(self, monkeypatch):
        ...
        assert result == expected_item, f"モックされたrandom.choicesが期待値'{expected_item}'を返しませんでした: {result}"
        print(f"[テスト成功] random.choicesが正しくモックされ、'{expected_item}'が返されました")