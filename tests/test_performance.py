...
class TestPerformance:
    ...
    @pytest.mark.parametrize("num_draws, max_time_sec", [
    ...
    def test_gacha_draw_performance(self, num_draws, max_time_sec):
        ...
        assert elapsed_time < max_time_sec, \
            f"{num_draws}回のガチャ処理が遅すぎます: {elapsed_time:.4f}秒 (目標: {max_time_sec:.4f}秒)"
        print(f"[テスト成功] {num_draws}回のガチャ処理が{elapsed_time:.4f}秒で完了しました (目標: {max_time_sec:.4f}秒以内)")